...
from transformers import EncoderDecoderModel, BertConfig, EncoderDecoderConfig, GPT2Tokenizer, GPT2Config
import time
import tqdm
import logging
import gc
...
def evaluate(data_loader, e_loss, should_print=True):
    ...
    with torch.no_grad():
        for step, batch in enumerate(data_loader):
            ...
            outputs = softmax(logits); #print(f"logits: {torch.sum(logits)}")
            ...
            for i in range(len(predicted)):
                if predicted[i] != batch[1][i]:
                    predicted_sents.append(predicted[i])
                    target_sents.append(batch[1][i])

            print("Sentences not matching")
            print(f"Predicted Sentences: {predicted_sents}")
            print(f"Target Sentences: {target_sents}")
            print("-" * 100)
            print("\n")
...
def train(**kwargs):
    ...
    train_dataset_l1 = WikiDataset(kwargs['base_path'] + kwargs['src_train_level_1'],
                                   kwargs['base_path'] + kwargs['tgt_train_level_1'])
    train_dataset_l2 = WikiDataset(kwargs['base_path'] + kwargs['src_train_level_2'],
                                   kwargs['base_path'] + kwargs['tgt_train_level_2'])
    train_dataset_l3 = WikiDataset(kwargs['base_path'] + kwargs['src_train_level_3'],
                                   kwargs['base_path'] + kwargs['tgt_train_level_3'])
    ...
    train_dl_1 = DataLoader(train_dataset_l1, batch_size=TRAIN_BATCH_SIZE, collate_fn=collate_fn, shuffle=True)
    train_dl_2 = DataLoader(train_dataset_l2, batch_size=TRAIN_BATCH_SIZE, collate_fn=collate_fn, shuffle=True)
    train_dl_3 = DataLoader(train_dataset_l3, batch_size=TRAIN_BATCH_SIZE, collate_fn=collate_fn, shuffle=True)
    valid_dl = DataLoader(valid_dataset, batch_size=TRAIN_BATCH_SIZE, collate_fn=collate_fn, shuffle=True)

    # Print number of batches in both train and valid dataloader
    print(f"Number of batches in train dataloaders: {len(train_dl_1)}, {len(train_dl_2)}, {len(train_dl_3)}")
    print(f"Number of batches in valid dataloader: {len(valid_dl)}")
    ...
    train_model(0, eval_loss,
                (train_dl_1, train_dl_2, train_dl_3, valid_dl), optimizer,
                kwargs['base_path'] + kwargs["checkpoint_path"],
                kwargs['base_path'] + kwargs["best_model"])
...
def train_model(start_epoch, eval_loss, loaders, optimizer, check_pt_path, best_model_path):
    ...
    print(f"Model training started with device: {device}")
    train_dl_1 = loaders[0]
    train_dl_2 = loaders[1]
    train_dl_3 = loaders[2]
    valid_dl = loaders[3]
    ...
    # epoch_eval_loss, bleu_score, sari_score = evaluate(valid_dl, eval_loss)
    # print(f"Initial eval loss: {epoch_eval_loss:.5f} | bleu score: {bleu_score} | sari score: {sari_score}")
    ...
    for epoch in range(start_epoch, N_EPOCH):
        ...
        epoch_eval_loss = 0
        model.train()
        batches = get_batches(train_dl_1, train_dl_2, train_dl_3)
        for step, batch in enumerate(batches):
            ...
            loss.backward()
            optimizer.step()
            loss_per_step.append(epoch_train_loss); print(f"loss per step: {loss_per_step[-1]}")
        ...
        epoch_eval_loss, bleu_score, sari_score = evaluate(valid_dl, epoch_eval_loss, True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", default="cp_ele_int/model_ckpt.pt", help="Checkpoint path")
    ...
    parser.add_argument("--train-type", default=None, help="Train type")
    ...
    if args.train_type:
        logging.info(f"Starting ssh for {args.train_type}")
        src_train = train_type_config[args.train_type]["src_train"]
        tgt_train = train_type_config[args.train_type]["tgt_train"]
        for train_args in train_type_config[args.train_type]:
            args.__dict__[train_args] = train_type_config[args.train_type][train_args]

        ssh_train(base_path=args.base_path, src_train=src_train, tgt_train=tgt_train,
                  src_valid=args.src_valid, tgt_valid=args.tgt_valid,
                  ref_valid=args.ref_valid, checkpoint_path=args.checkpoint_path,
                  best_model=args.best_model, seed=args.seed)